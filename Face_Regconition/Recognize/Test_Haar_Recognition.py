...
import imutils  # Thêm imutils từ file Haar Cascade
...
def extract_face(image_path, cascade_path="./HaarCascade/haarcascade_frontalface_default.xml"):
    ...
    if len(faces) > 0:
        ...
        x, y, w, h = faces[0]
        face = img[y:y+h, x:x+w]  # Cắt vùng khuôn mặt từ ảnh gốc
        face_rgb = cv2.cvtColor(face, cv2.COLOR_BGR2RGB)  # Chuyển sang RGB
        return face_rgb
    else:
        raise ValueError(f"Không tìm thấy khuôn mặt trong ảnh: {image_path}")

# Hàm vẽ khung và ghi chú trên ảnh sử dụng Haar Cascade
