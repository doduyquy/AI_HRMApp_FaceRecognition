...
def attendance(ui, best_match_name):
    ...
    if last_time is None or (current_time - last_time).total_seconds() > 10:
        ...
        if last_attendance is None:  # Chưa có bản ghi nào trong ngày hiện tại
            ...
            print(f"Check-in thành công cho {best_match_name}")
            # return tuple (True, f"Check-in thành công cho {best_match_name}")
            return True, f"Check-in thành công cho {best_match_name}"
            ...
            if time_diff >= MIN_CHECKOUT_DELAY:
                ...
                for item in ui.tree.get_children():
                    ...
                    if values[1] == f"NV{emp_id:03d}" and values[4] == "":
                        ...
                        break
                print(f"Check-out thành công cho {best_match_name}")
                # return tuple (True, f"Check-out thành công cho {best_match_name}")
                return True, f"Check-out thành công cho {best_match_name}"
            else:
                print(f"Chưa đủ {MIN_CHECKOUT_DELAY // 3600} giờ để check-out cho {emp_id}")
                # Return tuple (False, error_message)
                return False, f"Chưa đủ {MIN_CHECKOUT_DELAY // 3600} giờ để check-out cho {emp_id}"
        else:
            print(f"Đã hoàn tất check-in và check-out cho {emp_id} trong ngày hôm nay")
            # Return tuple (False, error_message)
            return False, f"Đã hoàn tất check-in và check-out cho {emp_id} trong ngày hôm nay"
