    ...
    if last_time is None or (current_time - last_time).total_seconds() > 10:
        ...
        if last_attendance is None:  # Chưa có bản ghi nào trong ngày hiện tại
            ...
            if time_diff >= MIN_CHECKOUT_DELAY:
                ...
                print
                print(f"Chưa đủ {MIN_CHECKOUT_DELAY // 3600} giờ để check-out cho {emp_id}-{best_match_name}")
                # Return tuple (False, error_message)
                return False, f"Chưa đủ {MIN_CHECKOUT_DELAY // 3600} giờ để check-out cho {emp_id}-{best_match_name}"
        else:
            print(f"Đã hoàn tất chấm công cho {best_match_id}-{best_match_name} trong ngày hôm nay")
            # Return tuple (False, error_message)
            return False, f"Đã hoàn tất chấm công cho {best_match_id}-{best_match_name} trong ngày hôm nay"
